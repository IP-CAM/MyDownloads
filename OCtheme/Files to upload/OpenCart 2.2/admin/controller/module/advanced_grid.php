...
class ControllerModuleAdvancedGrid extends Controller {
	...
	public function index() {   
		$this->language->load('module/advanced_grid');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/advanced_grid', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/advanced_grid', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Advanced Grid',
			'href' => $this->url->link('module/advanced_grid', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('module/advanced_grid', $data));
	}
