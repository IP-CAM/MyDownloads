...
class ControllerModuleMobileMenu extends Controller {
	...
	public function index() {   
		$this->language->load('module/mobile_menu');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/mobile_menu', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$data['action'] = $this->url->link('module/mobile_menu', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Mobile Menu',
			'href' => $this->url->link('module/mobile_menu', 'token=' . $this->session->data['token'], 'SSL')
		);
		...
		$this->response->setOutput($this->load->view('module/mobile_menu.tpl', $data));
	}
	
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/mobile_menu')) {
